 ...
 * @version V1.0
 */
public class Chomelink3 extends SpideEntryBase {
	...
	protected HtmlPage nextPage(HtmlPage page, int pageNum) {
		try {
			HtmlAnchor htmlNextPage = page.getAnchorByText("下一页");
	...
	protected void parsePage(HtmlPage Mainpage, Object linkItem, int pageNum) {
		...
		html = html.replaceAll("房源特色.*", " ");
		html = html.replaceAll(".*关注房源", " ");
		//
		dataSet.clear();
	...
	private void loadRegexRule() {
		...
		// 单价
		regexTable.set("price_unit", new CRegex("单价[:\\s]*([0-9.]+)\\s*元", 1), new CRegex("([0-9.]+)\\s*元/平米", 1));
		// 产权
		...
		regexTable.set("address", CRegexLib.addressRegex("地\\s*址[:\\s]*", ""), CRegexLib
		                .addressRegex("位于[:\\s]*", ""), CRegexLib.addressRegex("所在区域[:\\s]*", ""), CRegexLib.addressRegex("", ""));
		// 开发商
